          ...
          <div className="relative">
            <button type="button" className="-m-1.5 flex items-center p-1.5" onClick={() => setIsDropdownOpen(!isDropdownOpen)}>
              ...
              <div className="h-8 w-8 rounded-full bg-gray-300 flex items-center justify-center">
                {(user === null || user === void 0 ? void 0 : user.avatar_url) ? (<img className="h-8 w-8 rounded-full" src={user.avatar_url} alt={user.name}/>) : (<span className="text-sm font-medium text-gray-700">
                    {((_a = user === null || user === void 0 ? void 0 : user.name) === null || _a === void 0 ? void 0 : _a.charAt(0).toUpperCase()) || 'U'}
                  </span>)}
              </div>
            ...
            {/* Dropdown menu */}
            {isDropdownOpen && (<div className="absolute right-0 z-10 mt-2.5 w-32 origin-top-right rounded-md bg-white py-2 shadow-lg ring-1 ring-gray-900/5">
                <button onClick={handleLogout} className="block w-full px-3 py-1 text-left text-sm leading-6 text-gray-900 hover:bg-gray-50">
                  Sign out
                </button>
              </div>)}
          </div>
