...
export function Sidebar() {
  ...
  const { logo, tenantName, primaryColor } = useTheme();

  // Filter navigation based on tenant features
  const filteredNavigation = navigation.filter(item => {
    if (!item.feature) return true;
    return tenant?.settings?.features?.[item.feature as keyof typeof tenant.settings.features] ?? true;
  });
    ...
    <div className="hidden lg:fixed lg:inset-y-0 lg:flex lg:w-64 lg:flex-col">
      <div className="flex min-h-0 flex-1 flex-col bg-white border-r border-gray-200">
        ...
        <div className="flex h-16 flex-shrink-0 items-center px-4 border-b border-gray-200">
          <Link href={`/tenant/${tenantSlug}/dashboard`} className="flex items-center">
            {logo ? (
              <img className="h-8 w-auto" src={logo} alt={tenantName} />
            ) : (
