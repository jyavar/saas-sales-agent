...
function TenantGuard({ children }) {
    const { tenant, isLoading, error } = (0, TenantContext_1.useTenant)();
    ...
    if (error) {
        ...
        <div className="text-center max-w-md mx-auto px-4">
          ...
          <div className="space-y-3">
            <a href="https://stratoai.org" className="inline-block bg-blue-600 text-white px-6 py-2 rounded-md hover:bg-blue-700 transition-colors">
              Return to Strato AI
            </a>
            
            {error !== 'Tenant not found' && (<div>
                <button onClick={() => window.location.reload()} className="text-blue-600 hover:text-blue-800 underline text-sm">
                  Try again
                </button>
              </div>)}
          </div>
